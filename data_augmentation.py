...
def augment_flip(img,flip_type=1):
    ...
    img = np.array(img)
    if flip_type==0: #flip left/right ----> flip up/down
        flip_lr_img = cv2.flip(img,1)
        flip_lrud_img = cv2.flip(flip_lr_img,0)
        return flip_lrud_img
...
def Data_Augmentation(path,
    ...
    for img_f in pbar:
        ...
        if hsv_aug == True:
            bar_str += ' Hsv '
            hsv_images,hsv_he_images = augment_hsv(img,h_gain,s_gain,0.9,True,num_v)
            #print(hsv_images)
            #print(hsv_he_images)
...
def get_args():
    ...
    parser.add_argument('-imgdir','--img-dir',help='image dir',default='/home/ali/datasets/train_video/NewYork_train/train/aug_hsv/images/train')
