...
def save_flip_image(img_f,flip_lr_img,data_type=1,flip_type=1):
    ...
    if data_type==1 or data_type==0 :
        path_dir = os.path.join(path_dir_dir,'aug_flip','images','train')
    elif data_type==2:
        path_dir = os.path.join(path_dir_dir,'aug_flip','images','val')
...
def save_flip_label(img_f,data_type,flip_type):
    return True



def save_hsv_image_and_label(img_f,num_v,data_type,c,max_num,hsv_images):
    ...
    for i in range(num_v):
        if data_type == 0: #train/val
            if i%2==0 and i>0: # odd for train
                ...
                if c < max_num:
                    if i>=4 and i<= (num_v-2):
                        cv2.imwrite(path,hsv_images[i])
                 
                if i>3 and i<=(num_v - 2) : 
                    add = ''
                ...
                if c < max_num:
                    if i>=5 and i<=(num_v-1):
                        cv2.imwrite(path,hsv_images[i])
                 
                if i>3 and i<=(num_v-1):        
                    add = ''
                    label_f_2 = img2label_path(path,'labels',add)        
                    l2_dir,l2f,l2f_name,l2path_dir_dir = Analysis_Path(label_f_2)
                    if not os.path.exists(l2_dir):
...
def Data_Augmentation(path,
    ...
    for img_f in pbar:
        ...
        if hsv_aug == True:
            ...
            save_complete = save_hsv_image_and_label(img_f,num_v,data_type,c,max_num,hsv_images)
            c+=1
        '''================================Start flip augmentation==============================================='''
        if flip_aug == True:
            
            flip_lr_img = augment_flip(img,1)
            save_flip_image(img_f,flip_lr_img,data_type,flip_type)
            labell = save_flip_label(img_f,data_type,flip_type)
...
if __name__=="__main__":
    ...
    Data_Augmentation(path,
                      ...
                      True, # hsv augment
                      True, # flip augment
                      1, #flip type 0:lr+ud, 1:lr, 2:ud
                      0, # 0:train/val 1:train 2:val
                      18) # number of labels , remove front and tail labels
