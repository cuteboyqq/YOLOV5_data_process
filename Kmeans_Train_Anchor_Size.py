...
import matplotlib.pyplot as plt
# Plot
PREFIX = colorstr('Generate K-means of "x:num(cluster)--y:Avg.distance" Plot:')
k, d = [None]*anchor_num, [None]*anchor_num
for i in tqdm(range(1, anchor_num+1),desc=f'{PREFIX}plot'):
     k[i-1], d[i-1] = kmeans(wh / s, i,iter=30)  # points, mean distance
fig, ax = plt.subplots(1, 2, figsize=(14, 7), tight_layout=True)
ax = ax.ravel()
ax[0].plot(np.arange(1, anchor_num+1), np.array(d) , marker='.')
