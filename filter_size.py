...
def metric_bpr_aat(wh,k):  # compute metric
    thr = 4.0
    r = wh[:, None] / k[None]
...
def Auto_anchor(path,#path
    ...
    rimg = 0
    for label_file in tqdm(label_files,desc=f'{PREFIX}Start Analysis labels ri={rimg}:'):
        #print(c," :",label_file)
        c+=1
        #print('\r            <---------',end='\r')
        #print('\r',c,end='\r')
    ...
    #wh0 = np.concatenate([l[:, 3:5] for l in label_list ])  # wh
    wh0 = np.array(wh0)
    
    #scale = np.random.uniform(0.9, 1.1, size=(shapes.shape[0], 1))  # augment scale
    #wh = torch.tensor(np.concatenate([l[:, 3:5] * s for s, l in zip(shapes * scale,label_list)])).float()  # wh
    #bpr, aat = metric_bpr_aat(wh,init_anchor)
    ...
    for i in tqdm(range(1, K_means_max_iter+1),desc=f'{PREFIX}plot'):
         k[i-1], d[i-1] = kmeans(wh / s, anchor_num,iter=i)  # points, mean distance
...
def get_args():
    ...
    parser.add_argument('-imgdir','--img-dir',help='images dir',default='/home/ali/datasets/bdd100k_small/images/train')
    parser.add_argument('-numanchors','--num-anchors',type=int,default=9)
    parser.add_argument('-iter','--iter',type=int,default=35,help='kmeans max iters')
    parser.add_argument('-imgsize','--img-size',type=int,default=320,help='train img input size')
    parser.add_argument('-stopnum','--stop-num',type=int,default=200000,help='analysis max num of images')
    parser.add_argument('-gennum','--gen-num',type=int,default=3000,help='num iter of generic algorithm (GA)')
...
if __name__=="__main__":
    ...
    #path = "/home/ali/bdd100k_real_ori/images/train"
